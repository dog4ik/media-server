...
use crate::upnp::action::IntoArgumentList;
...
mod helper {
    ...
    use crate::upnp::service_variables::IntoUpnpValue;
    ...
    pub fn write_optional_property<T: Write>(
        w: &mut quick_xml::Writer<T>,
        ns: Ns,
        name: &str,
        value: &Option<impl IntoUpnpValue>,
    ) -> quick_xml::Result<()> {
        if let Some(val) = value {
            w.create_element(ns.as_str().to_owned() + name)
                .write_text_content(BytesText::new(&val.into_value().to_string()))?;
        }
        Ok(())
    }
}
...
pub mod properties {
    ...
    use upnp_class::{ContainerType, ItemType};
    ...
    impl DidlResponse {
        ...
        pub fn root() -> Self {
            ...
            let movies = Container::new("movies".into(), "0".into(), "Movies".into());
            let mut show = Item::new("show.0".into(), "0".into(), "cool show".into());
            let resource = Resource {
                uri: "http://localhost:6969/api/video/393/watch".into(),
                protocol_info: "http-get:*:video/matroska:*".into(),
                import_uri: None,
                size: Some(8899),
                duration: None,
                protection: None,
            ...
            show.set_property(resource);
            show.set_property(AlbumArtUri(
                "http://localhost:6969/api/episode/428/poster".into(),
            ));
            show.base.set_upnp_class(Some(ItemType::VideoItem(None)));
            Self {
    ...
    fn testing() {
        let mut item = Item::new("1".into(), "0".into(), "Test item".into());
        let mut container = Container::new("2".into(), "0".into(), "Test container".into());
        let art_uri = AlbumArtUri("hello".into());
        let storage_total = StorageTotal(0);
        item.set_property(art_uri.clone());
        container.set_property(storage_total);
        container.set_property(art_uri);
    }
}
...
pub struct Object {
    id: String,
    parent_id: String,
    /// The read-only required @restricted property indicates whether the object is
    /// modifiable. If set to “1”, the ability to modify or delete a given object is confined to the
    /// ContentDirectory service implementation. Therefore, a control point cannot add, modify or
    /// delete metadata from a restricted object. Additionally, control points are not able to add,
    /// modify or delete any children of a restricted container. However, the @restricted property
    /// does not propagate to descendant objects. Note however, that metadata of a restricted object
    /// can still change due to internal ContentDirectory service implementation manipulations.
    restricted: bool,
    dc_title: String,
    ...
    artist_discography_uri: Option<String>,
    lyrics_uri: Option<String>,
    relation: Option<String>,
    upnp_storage_medium: Option<String>,
    dc_description: Option<String>,
    upnp_long_description: Option<String>,
    upnp_icon: Option<String>,
    upnp_region: Option<String>,
    dc_rights: Option<String>,
    dc_date: Option<String>,
    dc_language: Option<String>,
    upnp_playback_count: Option<String>,
    upnp_last_playback_time: Option<String>,
    upnp_last_playback_position: Option<String>,
    upnp_recorded_start_date_time: Option<String>,
    upnp_recorded_end_date_time: Option<String>,
    upnp_recorded_duration: Option<String>,
    upnp_recorded_day_of_week: Option<String>,
    upnp_srs_record_schedule_id: Option<String>,
    upnp_srs_record_task_id: Option<String>,
    upnp_recordable: Option<bool>,
    upnp_program_title: Option<String>,
    upnp_series_title: Option<String>,
    upnp_program_id: Option<String>,
    upnp_series_id: Option<String>,
    upnp_channel_id: Option<String>,
    upnp_episode_count: Option<String>,
    upnp_episode_number: Option<String>,
    upnp_program_code: Option<String>,
    upnp_rating: Option<String>,
    upnp_recommendation_id: Option<String>,
    upnp_channel_group_name: Option<String>,
    upnp_call_sign: Option<String>,
    upnp_network_affiliation: Option<String>,
    upnp_service_provider: Option<String>,
...
impl ObjectType {
    pub fn upnp_class(&self) -> &'static str {
        match self {
            ObjectType::Item(i) => match &i.item_type {
                Some(ItemType::AudioItem(a)) => match a {
                    Some(AudioType::MusicTrack) => "object.item.audioItem.musicTrack",
                    None => "object.item.audioItem",
                },
                Some(ItemType::VideoItem(v)) => match v {
                    Some(VideoType::Movie) => "object.item.videoItem.movie",
                    None => "object.item.videoItem",
                },
                None => "object.item",
            },
            ObjectType::Container(c) => match c.container_type {
                Some(ContainerType::StorageFolder(_)) => "object.container.storageFolder",
    ...
    pub fn tag_name(&self) -> &'static str {
        match self {
            ObjectType::Item(_) => "item",
            ObjectType::Container(_) => "container",
        }
    }

    pub fn tag_attributes(&self) -> Vec<(&'static str, String)> {
        let mut attributes = Vec::new();
        match self {
            ObjectType::Item(_) => {}
            ObjectType::Container(c) => {
                if let Some(container_count) = c.child_container_count {
                    attributes.push(("childContainerCount", container_count.to_string()));
                };

                if let Some(searchable) = c.searchable {
                    attributes.push((
                        "searchable",
                        if searchable { "1".into() } else { "0".into() },
                    ));
                };

                if let Some(never_playable) = c.never_playable {
                    attributes.push((
                        "never_playable",
                        if never_playable {
                            "1".into()
                ...
                if let Some(child_count) = c.child_count {
                    attributes.push(("childCount", child_count.to_string()));
                }
            }
        };
        attributes
...
impl IntoXml for Object {
    fn write_xml(&self, w: &mut XmlWriter) -> quick_xml::Result<()> {
        let mut container_tag = BytesStart::new(self.object_type.tag_name()).with_attributes([
            ("id", self.id.to_string().as_str()),
            ("parentID", self.parent_id.to_string().as_str()),
            ("restricted", if self.restricted { "1" } else { "0" }),
        ]);
        let attributes = self.object_type.tag_attributes();
        container_tag.extend_attributes(attributes.iter().map(|(k, v)| (*k, v.as_str())));

        let container_tag_end = container_tag.to_end().into_owned();
        w.write_event(Event::Start(container_tag))?;
        w.create_element("dc:title")
            .write_text_content(BytesText::new(&self.dc_title))?;
        w.create_element("upnp:class")
            .write_text_content(BytesText::new(&self.object_type.upnp_class()))?;
        if let Some(dc_creator) = &self.dc_creator {
            w.create_element("dc:creator")
                .write_text_content(BytesText::new(dc_creator))?;
        }

        if let Some(write_status) = &self.write_status {
            w.create_element("upnp:writeStatus")
                .write_text_content(BytesText::new(&write_status.to_string()))?;
        }

        helper::write_optional_property(
            w,
            Ns::Upnp,
            "longDescription",
            &self.upnp_long_description,
        )?;
        helper::write_optional_property(w, Ns::Upnp, "producer", &self.producer)?;
        helper::write_optional_property(w, Ns::Upnp, "rating", &self.upnp_rating)?;
        helper::write_optional_property(w, Ns::Upnp, "actor", &self.actor)?;
        helper::write_optional_property(w, Ns::Upnp, "director", &self.director)?;
        helper::write_optional_property(w, Ns::Dc, "description", &self.dc_description)?;
        helper::write_optional_property(w, Ns::Dc, "publisher", &self.publisher)?;
        helper::write_optional_property(w, Ns::Dc, "language", &self.dc_language)?;
        helper::write_optional_property(w, Ns::Dc, "relation", &self.relation)?;
        helper::write_optional_property(w, Ns::Upnp, "playbackCount", &self.upnp_playback_count)?;
        // TODO: upnp:lastPlaybackTime@daylightSaving
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "lastPlaybackTime",
            &self.upnp_last_playback_time,
        )?;
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "lastPlaybackPosition",
            &self.upnp_last_playback_position,
        )?;
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "recordedStartDateTime",
            &self.upnp_recorded_day_of_week,
        )?;
        // TODO: upnp:lastPlaybackTime@daylightSaving
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "recordedStartDateTime",
            &self.upnp_recorded_start_date_time,
        )?;
        // TODO: upnp:recordedStartDate@daylightSaving
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "recordedEndDateTime",
            &self.upnp_recorded_end_date_time,
        )?;
        // TODO: upnp:recordedEndDate@daylightSaving
        helper::write_optional_property(
            w,
            Ns::Upnp,
            "recordedDuration",
            &self.upnp_recorded_duration,
        )?;
        helper::write_optional_property(
            w,
            Ns::Upnp,
...
#[derive(Debug, Default)]
pub struct Container {
    search_class: Vec<SearchClass>,
    create_class: Vec<CreateClass>,
    searchable: Option<bool>,
    never_playable: Option<bool>,
    child_container_count: Option<usize>,
    child_count: Option<usize>,
...
#[derive(Debug)]
pub enum ItemType {
    AudioItem(Option<AudioType>),
    VideoItem(Option<VideoType>),
}
...
#[derive(Debug)]
pub struct DidlBrowseResponse {
    objects: Vec<Object>,
}

impl DidlBrowseResponse {
    pub fn into_xml(&self) -> anyhow::Result<String> {
        let mut w = Writer::new(Vec::new());
        w.write_event(Event::Decl(BytesDecl::new("1.0", None, None)))?;
        let didl = BytesStart::new("DIDL-Lite").with_attributes([
    ...
    pub fn root() -> Self {
        ...
        //    res: vec![],
        //    write_status: None,
        //    object_type: ObjectType::Container(Container::default()),
        //};
        //let movies_container = Object {
        //    id: "movies".into(),
        //    parent_id: 0.to_string(),
        //    restricted: true,
    ...
    pub fn testing() -> Self {
        ...
        //let first_container = Object {
        //    id: "unique_id".to_string(),
        //    parent_id: 0.to_string(),
        //    restricted: false,
        //    dc_title: "My Music".into(),
        //    dc_creator: None,
        //    res: vec![],
        //    write_status: Some(WriteStatus::Writable),
        //    object_type: ObjectType::Container(Container {
        //        search_class: vec![
        //            SearchClass {
        //                class: "object.container.album.musicAlbum".into(),
        //                name: None,
        //                include_derived: Some(false),
        //            },
        //            SearchClass {
        //                class: "object.container.album.musicTrack".into(),
        //                name: None,
        //                include_derived: Some(false),
        //            },
        //        ],
        ...
        //        searchable: None,
        //        never_playable: None,
        //        container_type: Some(ContainerType::StorageFolder(StorageFolder {
        //            storage_used: 730_000,
        //        })),
        //        child_container_count: Some(2),
        //        child_count: Some(2),
        //    }),
        //};
        //objects.push(first_container);
        //
        //let second_container = Object {
        //    id: "uuid".into(),
        //    parent_id: 0.to_string(),
        //    restricted: false,
        //    dc_title: "My Photos".into(),
        //    dc_creator: None,
        //    res: vec![],
        //    write_status: Some(WriteStatus::Writable),
        //    object_type: ObjectType::Container(Container {
        //        search_class: vec![
        //            SearchClass {
        //                class: "object.container.album.photoAlbum".into(),
        //                name: None,
        //                include_derived: Some(false),
        //            },
        //            SearchClass {
        ...
        //    }),
        //};
        //objects.push(second_container);
        //let third_container = Object {
        //    id: 30.to_string(),
        //    parent_id: 0.to_string(),
        //    restricted: false,
        //    dc_title: "Album Art".into(),
        //    dc_creator: None,
        //    res: vec![],
        //    write_status: Some(WriteStatus::Writable),
        //    object_type: ObjectType::Container(Container {
        //        search_class: vec![SearchClass {
        //            class: "object.item.imageItem.photo.vendorAlbumArt".into(),
        //            name: Some("Vendor Album Art".into()),
        ...
        //            storage_used: 40_000,
        //        })),
        //        child_container_count: Some(0),
        //        child_count: Some(2),
        //    }),
        //};
        //objects.push(third_container);
        //
        Self { objects }
    }
}
...
mod tests {
    ...
    use crate::upnp::content_directory::{
        Container, ContainerType, CreateClass, Object, ObjectType, SearchClass, StorageFolder,
        WriteStatus,
    };

    use super::DidlBrowseResponse;

    #[test]
    fn serialize_root_children() {
...
object.item.imageItem.photo.vendorAlbumArt
        ...
        //};
        //objects.push(third_container);
        let response = DidlBrowseResponse { objects }.into_xml().unwrap();
