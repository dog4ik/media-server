use anyhow::Context;
use axum::http::{HeaderMap, HeaderValue};
use reqwest::{Url, header::AUTHORIZATION};

use crate::config;

/// Resolve provod request headers and base url
///
/// returns Error when url or auth token are not found
pub fn client_config(provider: &str) -> anyhow::Result<(HeaderMap, Url)> {
    let (config::ProvodKey(provod_key), config::ProvodUrl(provod_url)) =
        config::CONFIG.get_values();
    let provod_key = provod_key.context("missing provod agent token")?;
    let provod_url = provod_url.context("missing provod agent url")?;
    let mut base_url = Url::parse(&provod_url).context("parse url")?;
    tracing::info!("Using provod agent for {} provider", provider);
    base_url.set_path(provider);
    let mut headers = HeaderMap::with_capacity(1);
    headers.insert(AUTHORIZATION, HeaderValue::from_str(&provod_key)?);
    Ok((headers, base_url))
}
