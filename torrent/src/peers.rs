                ...
                Some(Ok(peer_msg)) = self.stream.next() => {
                    match peer_msg {
                        ...
                        PeerMessage::Bitfield { .. } => {
                            return Err(PeerError::logic("Peer is sending bitfield".into()));
                        }
                        PeerMessage::Request {
                            index,
                            begin,
                            ...
                            PeerStatus {
                                ...
                                message_type: MessageType::Request {
                                    response: tx,
                                    block: Block { piece: index, offset: begin, length },
                                }
                            })
                            .await;
                            if let Ok(Some(bytes)) = rx.await {
                                let _ = self.send_peer_msg(
                                PeerMessage::Piece {
                                    index,
                                    begin,
                                    block: bytes
                        ...
                        PeerMessage::Piece {
                            ...
                            block,
                                ...
                                println!("got piece {} with size({})", index, block.len());
                                self.ipc.status_tx.send(
                                PeerStatus {
                                    peer_id: self.uuid,
                                    message_type: MessageType::Data {
                                        block: Block { piece: index, offset: begin, length: block.len() as u32},
                                        bytes: block,
                                    },
                                })
                                .await
