...
CREATE VIRTUAL TABLE IF NOT EXISTS metadata_fts USING fts5(title, plot, content='metadata', content_rowid='id');
CREATE TRIGGER IF NOT EXISTS metadata_ai AFTER INSERT ON metadata BEGIN
  INSERT INTO metadata_fts(rowid, title, plot) VALUES (new.id, new.title, new.plot);
END;
CREATE TRIGGER IF NOT EXISTS metadata_ad AFTER DELETE ON metadata BEGIN
  INSERT INTO metadata_fts(metadata_fts, rowid, title, plot) VALUES('delete', old.id, old.title, old.plot);
END;
...
CREATE TABLE IF NOT EXISTS actors (
  ...
  external_metadata_provider TEXT NOT NULL,
  imdb_id TEXT,
  poster TEXT
);
